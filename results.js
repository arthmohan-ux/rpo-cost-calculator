...
// Restrict phone inputs to digits, +, spaces, dashes only
function enforcePhoneChars(inputEl) {
  inputEl.addEventListener('input', function() {
    var pos = inputEl.selectionStart;
    var before = inputEl.value;
    // Keep only digits, +, spaces, dashes
    var cleaned = before.replace(/[^0-9+\s\-]/g, '');
    if (cleaned !== before) {
      inputEl.value = cleaned;
      // Adjust cursor position for removed chars
  ...
  inputEl.addEventListener('keydown', function(e) {
    ...
    if ([8, 9, 27, 13, 46, 37, 38, 39, 40, 36, 35].indexOf(e.keyCode) !== -1) return;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Restrict phone to numeric input only
  enforcePhoneChars(modalPhoneInput);
...
function sendLead(payload) {
  // Send to Google Sheet (same fetch pattern as the explain proxy so redirects work)
  var endpoint = CONFIG.leadEndpoint;
  if (endpoint && endpoint !== 'YOUR_ENDPOINT_HERE') {
    fetch(endpoint, {
      method: 'POST',
      ...
      try {
        ...
        if (data.status === 'ok') console.log('[Lead] Saved to sheet');
        else console.warn('[Lead] Sheet response:', data);
      } catch (e) {
        console.warn('[Lead] Non-JSON response:', raw.substring(0, 200));
      }
    }).catch(function(err) {
      console.warn('[Lead] Send failed:', err.message);
    });
  }
